...
for ite_th = 1:option.num_Ite
    ...
    accracy_Matrix(6,ite_th) = Classify_LogEK(log_CovDs,rand_Matrix,option);
    time_Matrix(6,ite_th) = toc;

    tic;    % CDL (COV-LDA)
    accracy_Matrix(7,ite_th) = Classify_CDL_LDA(log_CovDs,rand_Matrix,option);
    time_Matrix(7,ite_th) = toc;

    tic;    % CDL (COV-PLS)
    accracy_Matrix(8,ite_th) = Classify_CDL_PLS(log_CovDs,rand_Matrix,option);
